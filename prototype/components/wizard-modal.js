...
class ModalWizard {
  ...
  _handleClose() {
    ...
    if (isFirstStep) {
      ...
      confirmAction({ title: 'Are you sure?', message: 'Your progress will be lost.', confirmLabel: 'Close', danger: true }).then(ok => { if (ok) this.close(); });
  ...
  _trapFocus() {
    // Remove previous trap listener to prevent accumulation
    if (this._focusTrapHandler) {
      this._overlay.removeEventListener('keydown', this._focusTrapHandler);
    }
    ...
    this._overlay.addEventListener('keydown', this._focusTrapHandler);
  }
}
